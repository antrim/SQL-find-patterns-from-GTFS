...
SELECT string_agg( trips.trip_id::text, ', ' ORDER BY sequences.min_arrival_time ) AS trips_list, sequences.stops_pattern, stop_time_intervals,trips.agency_id,routes.route_id,routes.route_short_name,routes.route_long_name,directions.direction_label,headsigns.headsign
FROM trips
INNER JOIN (
...
INNER JOIN
 (
 SELECT min_arrival_time, min_trip_times.trip_id, string_agg(
