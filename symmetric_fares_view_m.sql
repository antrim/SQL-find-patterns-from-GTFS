...
/* PLEASE NOTE:
   "MIGRATE_" VERSON OF VIEWS HERE. SEE FILE symmetric_fares_view_pm.sql
   for "PLAY_MIGRATE_" version.
  */
 ...
 * view which depends on migrate_fare_rules_symmetric within a different 
 * source file than this one. 
 */
DROP VIEW views.migrate_fare_rules_symmetric CASCADE;
...
/* 
 ...
 --------------------------------------------------------------------------
  ...
  WITH counts AS 
  ( SELECT -2*count(*) AS c FROM views.migrate_fare_rules_symmetric
    UNION ALL
    SELECT -count(*) AS c FROM views.migrate_fare_rules_asymmetric
    UNION ALL
    SELECT count(*) AS c FROM migrate_fare_rules)
...
-- views.migrate_fare_rules_asymmetric. Ed 2016-06-26.
